...
typedef struct
{
	...
	struct nodLD* next, * prev;// 2 pointer a cate 4B legatura 8B
} nodLD;
...
//stergere pe 3 cazuri
void stergemCarteDupaTitlu(nodLD** capLD, nodLD** coadaLD, char* titluDeSters)
{
	//caz 1 nodul sters e primul
	...
	else //caz 2 nodul sters este ultimul
		if (strcmp((*coadaLD)->info.titlu, titluDeSters) == 0)
	{
		nodLD* temp = *capLD;
		*coadaLD = (*coadaLD)->next;
		if (*coadaLD != NULL)
			(*coadaLD)->prev = NULL;
		free(temp->info.titlu);
		free(temp->info.vectPreturi);
...
void main()
{
	...
	printf("\n--------------Stergere carte--------------\n");
	stergemCarteDupaTitlu(&capLD, &coadaLD, "Baltagul");
	traversareLista(capLD);
	printf("\n--------------Traversare lista inversa --------------\n");
	traversareListainvers(coadaLD);
